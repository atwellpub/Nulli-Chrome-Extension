...
var Profiles_Manage = ( function () {
	...
	var App = {
		...
		init: function ( rules ) {
			...
			var main_container = $('<div>', { class: "panel panel-default" });
			var panel_heading = $('<div>', { class: "panel-heading" , text: rules.heading });
			var table = $('<table>', { class: "table", id:"rule-table" });
		...
		display_text: function( key , setting ) {
			...
			var input = $('<input>' , { type: 'text' , class: 'form-control' , id: setting.id , placeholder: setting.placeholder , required:true })
		...
		display_radio: function( key,	setting ) {
			...
			Object.keys(options).forEach(function(key) {
				...
				var span = $('<label/>' , { class: 'radio-label' });
				var input = $('<input/>' , { type: 'radio' , name: setting.id , class: setting.id, value: options[key].value } );
				...
				var label = $('<span/>' , { text: options[key].label } );
				input.appendTo(span);
				label.appendTo(span);
				span.appendTo(html);
			});
		...
		display_textarea: function( key , setting ) {
			var input = $('<textarea>' , { class: 'form-control' , id: setting.id	})
		...
		display_fileselect: function( key , setting ) {
			...
			var div_1 = $('<div>' , { class: 'fileupload fileupload-new' }).attr('data-provides' , 'fileupload');
			var span_1 = $('<span>' , { class: 'btn btn-primary btn-file' });
			var span_2 = $('<span>' , { class: 'fileupload-new' , text: i18n.get("Select File") });
			var span_3 = $('<span>' , { class: 'fileupload-exists' , text: i18n.get("Change") });
			var span_4 = $('<span>' , { class: 'fileupload-preview' , style: 'padding-left:10px;'});
			var input_1 = $('<input>' , { type: 'file' , id: setting.id });
			var link_1 = $('<a>' , { href: '#' , class: "close fileupload-exists" , text: "x" , style: 'float:none;padding-left:10px;'}).attr('data-dismiss' , 'fileupload');
		...
		save_profiles: function() {
			...
			/* Log object to console */
			console.log('Saves:');
			console.log({"nulli" : Profiles_Manage.nulli });

			chrome.storage.sync.set( {"nulli" : Profiles_Manage.nulli } );
		},
	};
